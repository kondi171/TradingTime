...
use restapi\Database\EntityInterface;
use restapi\Database\EntityMapper;
...
class ObjectModel {
    ...
    public function validateField($field, $value, $skip = array(), $human_errors = false)
    {
        ...
        if (!in_array('validate', $skip) && !empty($data['validate'])) {
            if (!method_exists('restapi\Validate', $data['validate'])) {
                throw new \Exception(
            ...
            if (!empty($value)) {
                ...
                if (Tools::strtolower($data['validate']) == 'iscleanhtml') {
                    if (!call_user_func(array('restapi\Validate', $data['validate']), $value, $hp_allow_html_iframe)) {
                        $res = false;
                    }
                } else {
                    if (!call_user_func(array('restapi\Validate', $data['validate']), $value)) {
                        $res = false;
                    }
                }
                if (!$res) {
